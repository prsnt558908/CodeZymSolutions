# Walls and Gates with the Most Assigned Rooms in Java
#### Problem Statement
...
That single sweep is a **multi source BFS**, and it is the only real "pattern" this problem needs. We pair it with one small structural choice: flatten the 2D grid into a 1D array so that each gate has a single integer id, which lets us store ownership in one plain `int` per cell.
...
**Strategy for the tie-break.** It is tempting to wrap the rule "smaller row, then smaller column" into a comparator object and call it whenever two gates compete for the same room. It would work, and in a codebase where the rule changed often it would be the right call. Here it buys nothing, because seeding the queue in sorted order deletes the comparison entirely. A pattern whose only job is to run a rule we can remove is pure overhead, and it would sit inside the hottest loop in the program.

**Dijkstra with a priority queue.** Shortest distance plus a preference sounds like a job for a priority queue. But Dijkstra exists to handle edges of different costs, and here every move costs exactly one. A plain FIFO queue already produces cells in distance order, so a priority queue would add a `log` factor and a heap of objects to achieve the ordering we already have.

What we do keep is the flattening trick, where cell `(r, c)` becomes index `r * cols + c`. It is small, but it is what lets a gate's identity live in a single `int`, which in turn lets ownership, counting and the queue all be plain `int` arrays instead of objects.

---

## Data Structures and Why Each One Is There

**`int[] cell`, the flattened grid.** Storing the grid as one array instead of a 2D array turns every cell into a single number. Neighbour math stays simple integer arithmetic, and more importantly a gate's position becomes an id we can use directly as an array index.

**`int[] owner`, one entry per cell.** It holds the flat index of the gate that claimed the cell, or `-1` when nobody has. Because "claimed" and "visited" mean the same thing in this BFS, this array doubles as the visited marker and we never need a separate one.

**`int[] roomsOwned`, indexed by the gate's own flat index.** A `HashMap<String, Integer>` keyed by `"row,col"` would also work, but we already have an integer id per gate, so a plain array is both simpler to read and faster, with no string building inside the loop.

**`int[] queue` with `head` and `tail`.** Every cell is pushed at most once, so `rows * cols` slots are always enough and the queue can never overflow. Using an array instead of `ArrayDeque<int[]>` avoids allocating a small object for each of the 250,000 cells.

**`List<Integer> gates`.** Collected during the row by row scan, so it is already in ascending row and then column order. That means the final answer needs no sorting, we just walk this list and keep the gates whose count equals the maximum.
 ...
 */
public class WallsAndGatesMostAssignedRooms {

    private static final int WALL = -1;
    private static final int GATE = 0;

    // Moves allowed from a cell: up, down, left and right.
    private static final int[] DR = {-1, 1, 0, 0};
    private static final int[] DC = {0, 0, -1, 1};

    public WallsAndGatesMostAssignedRooms() {
    }

    public List<String> findMostCommonGates(List<String> grid) {
        List<String> answer = new ArrayList<>();
        if (grid == null || grid.isEmpty()) return answer;

        int rows = grid.size();
        int cols = grid.get(0).split(",").length;
        int[] cell = parse(grid, rows, cols);

        // owner[i] = flat index of the gate that claimed cell i, -1 if unclaimed.
        // This also acts as the visited marker, so no separate array is needed.
        int[] owner = new int[rows * cols];
        Arrays.fill(owner, -1);

        // Room counter for each gate, indexed by the gate's own flat index.
        int[] roomsOwned = new int[rows * cols];

        // Every cell is pushed at most once, so a plain array is a safe queue.
        ...
        int best = 0;
        for (int gate : gates) best = Math.max(best, roomsOwned[gate]);

        for (int gate : gates) {
            if (roomsOwned[gate] == best) {
                answer.add((gate / cols) + "," + (gate % cols));
            }
        }
        return answer;
    }

    /** Flattens the comma separated rows into a single row major int array. */
    private int[] parse(List<String> grid, int rows, int cols) {
        int[] cell = new int[rows * cols];
        for (int r = 0; r < rows; r++) {
            String[] parts = grid.get(r).split(",");
            for (int c = 0; c < cols && c < parts.length; c++) {
                cell[r * cols + c] = Integer.parseInt(parts[c].trim());
            }
        }
        return cell;
    }
...
**Space: O(rows * cols).** Four integer arrays of one entry per cell, for the grid, the owner, the counters and the queue.
