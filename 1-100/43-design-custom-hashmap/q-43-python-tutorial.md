...
# Design a Custom HashMap in Python
...
The square in the statement means multiplication. Python's `^` operator performs bitwise XOR, so we must use `length * length`.


### Why These Classes and Lists?

- **`Entry`** is a dataclass that keeps one key and its value together. The map never changes an entry's key, while an update can replace its value.
- **`self._buckets`**, a list of lists, holds the buckets. Each inner list stores entries that share a bucket index.
- **`self._entry_count`** tracks the number of distinct keys, making `size()` constant time.


The solution uses Python lists and does not use a built-in dictionary or set. Each bucket must be a separate list. The helper creates a new list for every bucket, avoiding the shared-list problem caused by `[[]] * count`.
...
With four buckets, `"a"` and `"abcd"` both belong to bucket `2`. They still represent different entries. We scan that bucket and compare keys using `==`.
...
Round both constructor thresholds once using `floor(value * 100.0 + 0.5) / 100.0`. Apply the same rounding to `self._entry_count / bucket_count` before every threshold comparison. Python's `/` performs true division.


Do not use Python's built-in `round(value, 2)` here. It rounds exact halfway cases to the nearest even result. For example, `round(0.125, 2)` gives `0.12`, while the problem requires `0.13`. The `_round2` helper follows the rounding rule given in the statement.
...
First calculate the target bucket count. Then create the new buckets and move each existing entry to `_hash(key) % target_count`. Intermediate bucket layouts are not observable, so we can redistribute entries once after choosing the final count.
...
class CustomHashMap:
    ...
    def __init__(self, minLoadFactor: float, maxLoadFactor: float):
        self._min_load_factor = self._round2(minLoadFactor)
        self._max_load_factor = self._round2(maxLoadFactor)
        self._buckets = self._create_buckets(self.MIN_BUCKETS)
        self._entry_count = 0
    ...
    def getBucketKeys(self, bucketIndex: int) -> List[str]:
        keys: List[str] = []

        if bucketIndex < 0 or bucketIndex >= len(self._buckets):
            return keys

        for entry in self._buckets[bucketIndex]:
    ...
    def _hash(key: str) -> int:
        ...
        for i in range(length):
            result += ord(key[i]) - ord("a") + 1

        return result

    def _load_factor(self, bucket_count: int) -> float:
        # Use true division, then round before comparing thresholds.
        return self._round2(self._entry_count / bucket_count)

    @staticmethod
    def _create_buckets(count: int) -> List[List[Entry]]:
    ...
    def _check_rehash(self) -> None:
        target_count = len(self._buckets)

        # Choose one direction for this check to avoid resize oscillation.
        if self._load_factor(target_count) > self._max_load_factor:
            while self._load_factor(target_count) > self._max_load_factor:
                target_count *= 2
        elif self._load_factor(target_count) < self._min_load_factor:
            while (
                target_count > self.MIN_BUCKETS
                and self._load_factor(target_count) < self._min_load_factor
            ):
                target_count //= 2

        if target_count != len(self._buckets):
            self._rehash(target_count)

    def _rehash(self, new_bucket_count: int) -> None:
        new_buckets = self._create_buckets(new_bucket_count)

        for bucket in self._buckets:
            for entry in bucket:
                new_index = self._hash(entry.key) % new_bucket_count
